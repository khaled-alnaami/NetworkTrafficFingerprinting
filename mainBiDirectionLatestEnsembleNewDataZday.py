...
from KddClassifier import KddClassifier
...
def usage():
    ...
    -o: HP_dataset name (see config.HP_DATASETS)

    -R: Binary classification. HP_datasets only.

    """

def run():
    try:
        opts, args = getopt.getopt(sys.argv[1:], "t:T:N:k:c:C:d:n:r:B:D:E:F:G:H:I:J:K:L:M:A:m:V:P:g:q:x:b:l:u:s:X:i:p:w:W:Q:y:f:e:a:o:R:h")
...
###
        ...
        for clsfr in config.CLASSIFIER_LIST:
            ...
            if config.HP_BINARY != 0:
                outputFilenameArray.append('R'+str(config.HP_BINARY))
            ...
            for webpageId in webpageIds:
                ...
                for w in webpageList: # was for w in [webpageTrain, webpageTest]:
                    for trace in w.getTraces():
                        ...
                        # works for ids datasets only
                        if (config.HP_BINARY != 0 and (config.DATA_SOURCE == 63 or config.DATA_SOURCE == 64 or config.DATA_SOURCE == 65)):
                            if (traceWithCountermeasure.getId() in benignWebpageIds):
                                traceWithCountermeasure.setId(benignWebpageIds[0])
                            elif (traceWithCountermeasure.getId() in attackWebpageIds):
                                traceWithCountermeasure.setId(attackWebpageIds[0])
