...
class AdversarialClassifierBiDirectionFeaturesOnly:
    ...
    def traceToInstance( trace ):
        ...
        timeBase = 1
        if (config.DATA_SOURCE == 5 or config.DATA_SOURCE == 9): timeBase = config.bucket_Time # works well with Wang Tor
    ...
    def classify( runID, trainingSet, testingSet ):
        ...
        if config.OC_SVM == 0: # multi-class svm
            if config.CROSS_VALIDATION == 0:
                ...
                return wekaAPI.executeSklearnCrossValidation(file, classifier, config.CROSS_VALIDATION,
                                              **kwargs
                                              )
        else: # one-class svm
            if config.CROSS_VALIDATION == 0:
